...
//#include "MyUnit/ui_switchbutton.h"
#include "qpainter.h"
...
SwitchButton::SwitchButton(QWidget *parent): QWidget(parent)
{
    ...
    textOn = "开启";
    //step = 0;
    step = sizeHint().width() / 50;
    startX = 0;
    endX = 0;

    //timer = new QTimer(this);
    //timer->setInterval(30);
    //timer->setSingleShot(true);//设置为单次触发，执行后自动停止
    //connect(timer, SIGNAL(timeout()), this, SLOT(updateValue()));

    // 初始化属性动画（控制startX的变化）
    PropertyAnimation = new QPropertyAnimation(this, "startX", this);
    PropertyAnimation->setDuration(300); // 固定动画时长300ms（可根据需要调整）
    PropertyAnimation->setEasingCurve(QEasingCurve::InOutQuad); // 平滑过渡曲线
...
{
    if (PropertyAnimation->state() == QAbstractAnimation::Running) {
        return; // 动画运行时忽略点击，避免冲突
    }
...
{
    //如果刚刚初始化完成的属性改变则延时处理
    if (this->checked == checked) return; // 避免重复调用
    ...
    // 直接计算endX和启动定时器（复用mousePressEvent中的逻辑）
    step = width() / 7;
...
int SwitchButton::getStartX() const
{
    return startX;
}

void SwitchButton::setStartX(int value)
{
    startX = value;
    update(); // 每次更新坐标后重绘
}
