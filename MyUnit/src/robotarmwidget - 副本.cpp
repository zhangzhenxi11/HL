...
void RobotArmWidget::pickWafer(int stationId, int armIndex)
{
    ...
    if (!parallelAnimationGroup)
    {
        parallelAnimationGroup = new QParallelAnimationGroup(this);
    }
    parallelAnimationGroup->clear();
    ...
    if (armIndex == 1)
    {
        ...
        connect(parallelAnimationGroup, &QParallelAnimationGroup::finished, this, [=]() {
            // 设置持有晶圆标志
            setHasWafer(true);
    ...
    //取片动作动画
    parallelAnimationGroup->addAnimation(shoulderAnim);
    parallelAnimationGroup->addAnimation(elbowAnim);
    //缩回动作动画
