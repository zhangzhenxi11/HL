...
/**
 * 开关按钮控件 作者:zhangzhenxi(QQ:1595187426) 2016-11-6
...
class MYCUSTOMLIB_API SwitchButton : public QWidget

{
    ...
    Q_PROPERTY(int startX READ getStartX WRITE setStartX)
...
private:
    ...
    int startX;                     //滑块开始X轴坐标
    int endX;                       //滑块结束X轴坐标
    QPropertyAnimation* PropertyAnimation;  // 新增：属性动画对象
    QTimer *timer;                  //定时器绘制
...
public Q_SLOTS:
    ...
    int getStartX() const;          // 新增：获取startX
    void setStartX(int value);      // 新增：设置startX（用于动画）
