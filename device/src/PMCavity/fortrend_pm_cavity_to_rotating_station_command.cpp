...
PMCavityToRotatingStationCommand::RunResult PMCavityToRotatingStationCommand::onRun() throw(KernelException){
	...
	{
		throw KernelCommandRejectException(__FILE__, KernelSysException::KR_MODULE_RESPONSE_ERROR, Poco::format(" %s 读设置的旋转位命令地址错误，地址%s", sub->getName(), axis_target3_position_address), this);
	}
