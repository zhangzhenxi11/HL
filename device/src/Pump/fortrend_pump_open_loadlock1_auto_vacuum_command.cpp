...
namespace FC{
	...
	*/
	PumpOpenLoadLock1AutoVacuumCommand::PumpOpenLoadLock1AutoVacuumCommand(KeyencePlcSubSystemHelper* helper)
		...
		//setMessageName("OpenLoadLock1AutoVacuum");
		//setDescription("open loadlock1 auto cauum for pump");
	};
	...
	*/
	PumpOpenLoadLock1AutoVacuumCommand::RunResult PumpOpenLoadLock1AutoVacuumCommand::onRun() throw(KernelException){
		...
		{
			throw KernelCommandRejectException(__FILE__, KernelSysException::KR_MODULE_DOOR_EXCEPTION, Poco::format("子系统: %s 放晶圆盒的门已打开（逻辑错误）", lk1->getName()), this);
		}
		if (lk1->getTMCavityDoorOpend())
		{
			throw KernelCommandRejectException(__FILE__, KernelSysException::KR_MODULE_DOOR_EXCEPTION, Poco::format("子系统: %s 传输腔门阀已打开（逻辑错误）", lk1->getName()), this);
		}*/
		...
		logInform(pump->getName().c_str(), Poco::format("打开%s真空命令开始", lk1->getName()).c_str());
		bool loop = true;
		...
		while (loop)
		{
			...
			case 100:
			{
				...
				if (lk1->getLoadLockRoughVacuumReachesTheSetValue() == false || pump->getMolecularPumpReachSpeedLLA() == false)// || 
				{
					step = 1000;
				}
				else if (lk1->getFastDiaphragmValveOpend() || lk1->getSlowDiaphragmValveOpend())//隔膜阀打开或PID打开 lk1->getPIDOpend() || 
				{
					step = 1010;
				}
				else if (lk1->getInsertingPlateValveOpend() == false)//插板阀关闭
				{
					step = 1300;
				}
				else
			...
			case 1000:
			{
				...
				{
					//关闭插板阀
					if (lk1->getInsertingPlateValveOpend())
					{
						...
						if (cmd->hasError())
						{
							addCommandExecutionAlarmMessage(step, lk1->getName(), "关闭插板阀");
							step = 10000;
						}
						else
						{
			...
			case 1010:
			{
				/*if (lk1->getState() == IKernelSubSystem::State::SUB_NORMAL)
				{
					if (lk1->getPIDOpend())
					{
						auto cmd = lk1->createClosePIDCommand();
						lk1->startCommand(cmd);
						cmd->wait();
						if (cmd->hasError())
						{
							addCommandExecutionAlarmMessage(step, lk1->getName(), "关闭PID");
							step = 10000;
						}
						else{
							step = 1030;
			...
			case 1030:
			{
				if (lk1->getState() == IKernelSubSystem::State::SUB_NORMAL)
				{
					if (lk1->getFastDiaphragmValveOpend() || lk1->getSlowDiaphragmValveOpend())
					{
						//关闭隔膜阀
						...
						if (cmd->hasError())
						{
							addCommandExecutionAlarmMessage(step, lk1->getName(), "关闭隔膜阀");
							step = 10000;
						}
						else
						{
			...
			case 1045:
			{
				if (lk1->getState() == IKernelSubSystem::State::SUB_NORMAL)
				{
					if (lk1->getCassetteDoorOpend())
					{
						...
						if (cmd->hasError())
						{
							addCommandExecutionAlarmMessage(step, lk1->getName(), "关闭晶圆盒门");
							step = 10000;
						}
						else
						{
			...
			case 1050:
			{
				if (tm->getState() == IKernelSubSystem::State::SUB_NORMAL)
				{
					if (tm->getAngleValveOpend() && lk1->getVacuumValue() > 300)
					{
			...
			case 1055:
			{
				if (tm->getState() == IKernelSubSystem::State::SUB_NORMAL)
				{
					if (tm->getHeightVacuumBaffleValveOpend() && lk1->getVacuumValue() > 300)
					{
			...
			case 1060:
			{
				if (lk2->getState() == IKernelSubSystem::State::SUB_NORMAL)
				{
					if (lk2->getAngleValveOpend() && lk1->getVacuumValue() > 300)
					{
			...
			case 1065:
			{
				if (lk2->getState() == IKernelSubSystem::State::SUB_NORMAL)
				{
					if (lk2->getHeightVacuumBaffleValveOpend() && lk1->getVacuumValue() > 300)
					{
			...
			{
				//是否达到粗抽压力
				if (lk1->getLoadLockRoughVacuumReachesTheSetValue() == false)
				{
					step = 1120;
				}
				else
			...
			{
				//分子泵打开或者运行状态
				if (pump->getMolecularPumpOpenedLLA() || (pump->getMolecularPumpRunningStateLLA() == 1))
				{
					step = 1140;
				}
				else
			...
			case 1200:
			{
				...
				{
					//打开高真空挡板阀
					if (lk1->getHeightVacuumBaffleValveOpend() == false)
					{
						auto cmd = lk1->createOpenHeightVacuumBaffleValveCommand();
						lk1->startCommand(cmd);
						cmd->wait();
						if (cmd->hasError())
						{
							addCommandExecutionAlarmMessage(step, lk1->getName(), "打开高真空挡板阀");
							step = 10000;
						}
						else
						{
			...
			{
				//达到粗抽压力
				if (lk1->getLoadLockRoughVacuumReachesTheSetValue())
				{
					Sleep(1000);
					step = 1300;
				}
			...
			case 1300:
			{
				if (lk1->getLoadLockRoughVacuumReachesTheSetValue(10)){
			...
			case 1305:
			{
				...
				{
					//关闭高真空挡板阀
					if (lk1->getHeightVacuumBaffleValveOpend())
					{
			...
			case 1310:
			{
				if (lk1->getState() == IKernelSubSystem::State::SUB_NORMAL)
				{
					if (lk1->getAngleValveOpend() == false)
					{
						...
						if (cmd->hasError())
						{
							addCommandExecutionAlarmMessage(step, lk1->getName(), "打开角阀");
							step = 10000;
						}
						else
						{
			...
			case 1320:
			{
				if (tm->getMoleculePipelineVacuumValue() < 10.0)
				{
					if (pump->getMolecularPumpOpenedLLA() == false)
					{
						if (pump->getState() == IKernelSubSystem::State::SUB_NORMAL)
						{
							//打开分子泵
							auto cmd = pump->createMolecularOpenCommand("LLA");
			...
			case 1330:
			{
				if (pump->getMolecularPumpReachSpeedLLA())
				{
					step = 1340;
				}
				else
			...
			case 1340:
			{
				if (tm->getMoleculePipelineVacuumValue() < 5.0  && lk1->getAngleValveOpend())
				{
					if (pump->getMolecularPumpReachSpeedLLA())
					{
						step = 1345;
			...
			{
				Sleep(2000);
				if (lk1->getLoadLockRoughVacuumReachesTheSetValue(6))
				{
					step = 1355;
				}
				else
			...
			case 1360:
			{
				if (lk1->getState() == IKernelSubSystem::State::SUB_NORMAL)
				{
					if (lk1->getInsertingPlateValveOpend() == false)
					{
						if (lk1->getLoadLockRoughVacuumReachesTheSetValue(10)){
							//打开插板阀
							...
							if (cmd->hasError())
							{
								if (!lk1->getLoadLockRoughVacuumReachesTheSetValue(10)){
									...
									addCommandExecutionAlarmMessage(step, lk1->getName(), "打开插板阀");
									step = 10000;
								}
							}
							else{
			...
			case 5200:
			{
				//if (lk1->getState() == IKernelSubSystem::State::SUB_NORMAL)
				//{
				//	if (lk1->createCloseAngleValveCommand())
				//	{
				//		//关闭角阀
				//		auto cmd = lk1->createCloseInsertingPlateValveCommand();
				//		lk1->startCommand(cmd);
			...
			{
				AlarmMessage::Ptr alarm(new AlarmMessage(1, 10000, Poco::format("打开%s真空命令执行超时", lk1->getName())));
				setAlarm(alarm);
				step = 10000;
				loop = false;
				break;
		...
		{
			logInform(pump->getName().c_str(), Poco::format("打开%s真空命令执行完成", lk1->getName()).c_str());
		}
		return ret;
	}
