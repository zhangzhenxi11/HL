...
class EFEMLoadPortCloseDoorCommandPrivate{
public:
	std::string name;
};


EFEMLoadPortCloseDoorCommand::EFEMLoadPortCloseDoorCommand(HexSubSystemHelper* hexHelper)
:HexCommandExecuter(hexHelper)
, d(new EFEMLoadPortCloseDoorCommandPrivate){
	d->name = "CloseDoor";
	setMessageName(d->name);	
}


std::string EFEMLoadPortCloseDoorCommand::getName() const { 
	return d->name; 
};
...
*/
EFEMLoadPortCloseDoorCommand::RunResult EFEMLoadPortCloseDoorCommand::onRun() throw(KernelException){
