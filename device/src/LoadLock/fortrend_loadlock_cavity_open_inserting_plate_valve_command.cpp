...
namespace FC{
	...
	/**
	* LoadLockOpenInsertingPlateValveCommand
	*/
	LoadLockOpenInsertingPlateValveCommand::LoadLockOpenInsertingPlateValveCommand(KeyencePlcSubSystemHelper* hexHelper)
	...
	std::vector<IKernelResources* > LoadLockOpenInsertingPlateValveCommand::resources() const{
		return std::vector<IKernelResources* >();
	}
	/**
	* return true if success else false.
	*/
	LoadLockOpenInsertingPlateValveCommand::RunResult LoadLockOpenInsertingPlateValveCommand::onRun() throw(KernelException){
		...
		if (sub->getCassetteDoorOpend() && sub->getTMCavityDoorOpend())
		{
			throw KernelCommandRejectException(__FILE__, KernelSysException::KR_SYSTEM_LOGIC_ERROR, Poco::format("子系统: %s 晶圆盒门和传输腔门未关闭（逻辑错误）", sub->getName()), this);
		}
		...
		if (!sub->getLoadLockRoughVacuumReachesTheSetValue(10))
		{
