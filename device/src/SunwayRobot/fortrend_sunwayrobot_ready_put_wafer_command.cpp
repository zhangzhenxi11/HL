...
SunwayRobotReadyPutWaferCommand::RunResult SunwayRobotReadyPutWaferCommand::onRun() throw(KernelException){
	...
	if (timeout < 10) {
		throw KernelCommandRejectException(__FILE__, KernelSysException::KR_COMMON_DATA_OUTOF_RANGE,
			Poco::format("超时: %s 准备放晶圆超时参数错误.", robot->getName()), this);
	}

	std::string str_arm = (getArm() == 0) ? "A" : "B";
	std::string command = "MOV:RETREATHPOS/";
	...
	const std::string commandContext = "ReadyPut-" + std::to_string(getStation()->getStationId(robot->getName())) + "-" + std::to_string(getSlot());
	...
	logInform(robot->getName().c_str(), "准备放晶圆ACK:%s", res.c_str());
	...
	while (true)
	{
		...
		if (elapsed >= timeoutWindow)
		{
			AlarmMessage::Ptr alarm(new AlarmMessage(1, 0x100, "准备放晶圆返回指令超时."));
			setAlarm(alarm);
			return RunResult::RUN_FAILD;
		}
		res = recvResponseRobotMessageMatching(timeout,
	...
	logInform(robot->getName().c_str(), "准备放晶圆RPS:%s", res.c_str());
