...
namespace FC{
	...
	private:
		...
		QFortrendSlotWidget* slot_widgetLp1;
		QFortrendSlotWidget* slot_widgetLp2;
		...
		int ewtrStationIdeAligner = 4;
		int ewtrStationIdeLk1 = 5;
		int ewtrStationIdeLk2 = 6;
	...
	QFortrendStationStatusVTMWidgetPrivate::QFortrendStationStatusVTMWidgetPrivate(QFortrendStationStatusVTMWidget* p, const std::shared_ptr<IKernel>& kernel)
		...
		//25-11-05 ADD
		erobotdialog = new ERobotDialog(p);
		erobotdialog->setWindowFlags(erobotdialog->windowFlags() | Qt::WindowStaysOnTopHint);
		...
		//这里加入会报错！
		ewtr->KernelAbstractSubSystem::addListener(this); //加入订阅队伍中
	/*	ewtrStationIdLp1 = lp1->getStationId(ewtr->getName());
		ewtrStationIdLp2 = lp2->getStationId(ewtr->getName());
		ewtrStationIdeAligner = ealigner->getStationId(ewtr->getName());*/
...
#pragma region 添加工位和手爪配置
		...
		//create efem arm select
		for (int i = 0; i < ewtr->armCount(); i++)
		{
			//select
			std::string name = ewtr->getArmName(i);
			QRadioButton* selectBtn = new QRadioButton(QString::fromStdString(name));
			selectBtn->setProperty("index", i);
			erobotdialog->addArm(selectBtn);
			if (i == 0)
				selectBtn->setChecked(true);
		}
#pragma endregion
	...
	void QFortrendStationStatusVTMWidgetPrivate::onKernelEvent(FC::KernelEventModule* kernelModule, const std::shared_ptr<FC::IEventId>& eventId, FC::KernelEventParameter* context){
		...
		if (std::dynamic_pointer_cast<EVENT_COMMAND_RUNNING>(eventId)){
			...
			{
				//int station = ealigner_align_command->getStation()->getStationId(ewtr->getName());
				//int arm = ealigner_align_command->getArm();
	...
	*/
	...
	QFortrendStationStatusVTMWidget::QFortrendStationStatusVTMWidget(const std::shared_ptr<IKernel>& kernel, QWidget* parent)
		...
		//25-11-5 ADD
		auto casslp1 = d->cassManager->getCassette(d->lp1.get());
		if (casslp1)
		{
			casslp1->setBoxId("LP1");
			d->slot_widgetLp1 = new QFortrendSlotWidget(casslp1);
			d->slot_widgetLp1->canSelected(true,false);
			d->slot_widgetLp1->setName(QString::fromStdString("ELP1"));
			d->ui->gridLayoutC->addWidget(d->slot_widgetLp1,0,0,Qt::AlignLeft);
			//QWidget* cassette_Widget = new QFortrendCassetteWidget(casslp1, d->cassManager, true, true, 25, this); //max row count = 25
			//d->ui->center_layout->addWidget(cassette_Widget);
		}

		auto casslp2 = d->cassManager->getCassette(d->lp2.get());
		if (casslp2)
		{
			casslp2->setBoxId("LP2");
			d->slot_widgetLp2 = new QFortrendSlotWidget(casslp2);
			d->slot_widgetLp2->canSelected(true, false);
			d->slot_widgetLp2->setName(QString::fromStdString("ELP2"));
			d->ui->gridLayoutD->addWidget(d->slot_widgetLp2, 0, 0, Qt::AlignLeft);
			//QWidget* cassette_Widget = new QFortrendCassetteWidget(casslp2, d->cassManager, true, true, 25, this); //max row count = 25
			//d->ui->center_layout->addWidget(cassette_Widget);
		}
...
#pragma endregion
#pragma region efem大气机械手添加工位选择

		for (int i = 0; i < d->ewtr->getWorkStations().size(); i++)
		{
			auto station = d->ewtr->getWorkStations().at(i);
			QRadioButton* radioButton = new QRadioButton;
			radioButton->setText(QString::fromStdString(station->getName()) + QString("[%1]").arg(station->getStationId(d->ewtr->getName())));
			radioButton->setProperty("index", i); //index
			d->erobotdialog->addStation(radioButton);
			if (i == 0)
			{
				radioButton->setChecked(true);
			}
			...
			QObject::connect(radioButton, &QRadioButton::clicked, this, [=]() {
				...
				// 获取对话框的布局
				QLayout* slotsLayout = d->erobotdialog->findChild<QLayout*>("slots_layout"); // 假设布局有对象名
...
#pragma endregion  efem机械手模块信号槽绑定
		QObject::connect(d->ui->ewtr_widget, &RobotArmWidget::signalERobotDialog, this, &QFortrendStationStatusVTMWidget::onERobotDialog);
		QObject::connect(d->ui->ewtr_widget, &RobotArmWidget::signalERobotReset, this, &QFortrendStationStatusVTMWidget::onERobotReset);
		QObject::connect(d->ui->ewtr_widget, &RobotArmWidget::signalERobotGetStatus, this, &QFortrendStationStatusVTMWidget::onERobotGetStatus);
		QObject::connect(d->ui->ewtr_widget, &RobotArmWidget::signalERobotClearError, this, &QFortrendStationStatusVTMWidget::onERobotClearError);
		QObject::connect(d->erobotdialog, &ERobotDialog::signalSpeed, this, &QFortrendStationStatusVTMWidget::onERobotSpeed);
		QObject::connect(d->erobotdialog,&ERobotDialog::signalGet,this,&QFortrendStationStatusVTMWidget::onERobotGet);
		QObject::connect(d->erobotdialog, &ERobotDialog::signalPut, this, &QFortrendStationStatusVTMWidget::onERobotPut);
#pragma region
...
{
	Q_D(QFortrendStationStatusVTMWidget);
	d->ui->ewtr_widget->setArmSpeed(2000);
	// 根据不同的动作类型执行相应的操作
	if (action == "get") {
		d->ui->ewtr_widget->queueAnimation("pick", station, arm);// "retract", "pick", "place", "extend"
		d->ui->ewtr_widget->queueAnimation("retract", station, arm);
	} 
	else if (action == "put") {
		d->ui->ewtr_widget->queueAnimation("place", station, arm);// "retract", "pick", "place", "extend"
		d->ui->ewtr_widget->queueAnimation("retract", station, arm);
	} 
...
#pragma region 机械手模块指令
	...
	void QFortrendStationStatusVTMWidget::onERobotDialog()
	{
		Q_D(QFortrendStationStatusVTMWidget);
		d->erobotdialog->show();
	}
	void QFortrendStationStatusVTMWidget::onERobotReset()
	{
		Q_D(QFortrendStationStatusVTMWidget);
		KernelSubsystemCommand::Ptr cmd = d->ewtr->createResetCommand();
		d->ewtrwidget->executeCommand(d->ewtr, cmd);
	}
	void QFortrendStationStatusVTMWidget::onERobotGetStatus()
	{
		Q_D(QFortrendStationStatusVTMWidget);
		KernelSubsystemCommand::Ptr cmd = d->ewtr->createUpdateCommand();
		d->ewtrwidget->executeCommand(d->ewtr, cmd);
	}
	void QFortrendStationStatusVTMWidget::onERobotClearError()
	{
		Q_D(QFortrendStationStatusVTMWidget);
		KernelSubsystemCommand::Ptr cmd = d->ewtr->createResetCommand();
		d->ewtrwidget->executeCommand(d->ewtr, cmd);
	}
	void QFortrendStationStatusVTMWidget::onERobotSpeed(int speed)
	{
		Q_D(QFortrendStationStatusVTMWidget);
		KernelSubsystemCommand::Ptr cmd = d->ewtr->createSetSpeedCommand(speed);
		d->ewtrwidget->executeCommand(d->ewtr, cmd);
	}
	void QFortrendStationStatusVTMWidget::onERobotGet(int station, int arm, int slot)
	{
		Q_D(QFortrendStationStatusVTMWidget);
		if (station == -1) {
			logWarn(d->ewtr->getName().c_str(), "请选择取料工位!");
			return;
		}
		if (arm == -1) {
			logWarn(d->ewtr->getName().c_str(), "请选择取料手爪!");
			return;
		}

		//bool armwafer = d->ewtr->hasObject(arm);
		//if (armwafer) {
		...
		std::shared_ptr<FortrendStation> stationptr = d->ewtr->getWorkStations().at(station);
		...
		if (station == 5) {
			...
			station = d->ewtrStationIdeLk1;
		}
		else if (station == 6) {
			stationIsWafer = stationcass->getMapping(d->ui->loadlockB_widget->GetCurrentSlot()) == Cassette::Present;
			station = d->ewtrStationIdeLk2;
		}
		else if (station == 1) {
			stationIsWafer = stationcass->getMapping(slot) == Cassette::Present;
			station = d->ewtrStationIdLp1;
		}

		else if (station == 2) {
			stationIsWafer = stationcass->getMapping(slot) == Cassette::Present;
			station = d->ewtrStationIdLp2;
		}
		else {
			stationIsWafer = stationcass->getMapping(1) == Cassette::Present;
		}
		//if (!stationIsWafer) {
		//	logWarn(d->ewtr->getName().c_str(), "工位%s没有料，禁止取料，请重新检查!", stationptr->getName());
		//	return;
		//}
