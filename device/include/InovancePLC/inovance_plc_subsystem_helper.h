...
#ifndef _XLH_FORTREND_INOVANCE_PLC_SUBSYSTEM_HELPER_INCLUDE_
#define _XLH_FORTREND_INOVANCE_PLC_SUBSYSTEM_HELPER_INCLUDE_
...
*/
class InovancePlcSubSystemHelper
{
public:
	DECLARE_PTR(InovancePlcSubSystemHelper)
	typedef struct{
		int type;
		int code;
		std::string message;
	...
	explicit InovancePlcSubSystemHelper(const std::string& name);
public:
...
protected:
	...
	*/
	void configInovancePlc(const std::shared_ptr<KernelConfiguration> & config);
...
private:
	DECLARE_PRIVATE(InovancePlcSubSystemHelper);
};
